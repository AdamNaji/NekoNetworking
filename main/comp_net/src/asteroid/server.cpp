    ...
    void Server::ReceivePacket(std::unique_ptr<asteroid::Packet> packet)
    {
        ...
        case asteroid::PacketType::JOIN:
        {
            ...
            if (lastPlayerNumber_ == asteroid::maxPlayerNmb)
            {
                ...
                auto spawnBulletPacket = std::make_unique<asteroid::SpawnBulletPacket>();
                spawnBulletPacket->packetType = asteroid::PacketType::SPAWN_BULLET;
                using namespace std::chrono;
                unsigned long ms = (duration_cast<milliseconds>(system_clock::now().time_since_epoch()) + milliseconds(3000)).count();
                startGamePacket->startTime = ConvertToBinary(ms);
                SendReliablePacket(std::move(startGamePacket));
                auto dir = Vec2f(RandomRange(-1.0f, 1.0f), RandomRange(-1.0f, 1.0f)).Normalized() * asteroid::bulletSpeed;
                spawnBulletPacket->pos = ConvertToBinary(Vec2f::zero);
                spawnBulletPacket->velocity = ConvertToBinary(dir);
                SendReliablePacket(std::move(spawnBulletPacket));
                gameManager_.SpawnBullet(Vec2f::zero, dir);
            }
                break;
            }
